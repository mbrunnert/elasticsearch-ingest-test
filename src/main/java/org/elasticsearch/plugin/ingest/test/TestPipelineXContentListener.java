...
public class TestPipelineXContentListener extends RestToXContentListener<SimulatePipelineResponse> {
     ...
     * @param expectedDocs
     */
    TestPipelineXContentListener(RestChannel channel, List expectedDocs) {
        super(channel);
    ...
    public RestResponse buildResponse(SimulatePipelineResponse response, XContentBuilder builder) throws Exception {
        ...
        List<Map<String, Object>> simulateResults = response.getResults().stream()
                .map(TestPipelineXContentListener::reformatSimulateResult).collect(Collectors.toList());

        List diffObject = createDiff(simulateResults);

        TestPipelineResponse decoratedResponse = new TestPipelineResponse(response, diffObject);
